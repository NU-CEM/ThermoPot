...
class Calculation(object):
    ...
    def check_attributes(self):
        ...
        assert ((type(self.NAtoms) == int) and self.NAtoms >= 1), "NAtoms must be an integer >= 1"
        assert ((type(self.volume) == float) and self.volume > 0), "volume must be a float > 0"
...
class AimsCalculation(Calculation):
    ...
    def get_volume(self):
        ...
        with open(self.filepath) as contents:
            return float(re.findall("Unit cell volume\s+:\s*(.*)\sA",contents.read())[-1])

    def get_energy(self):

        with open(self.filepath) as contents:
            return float(re.findall("Total energy of the DFT[^0-9]+(-\d*\.?\d*) eV",contents.read())[-1])
    ...
    def get_NAtoms(self):
        ...
        with open(self.filepath) as contents:
            return int(re.findall("Number of atoms\s +:\s + (\S+)", contents.read())[-1])
